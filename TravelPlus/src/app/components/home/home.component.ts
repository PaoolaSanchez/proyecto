...
export class HomeComponent implements OnInit {
  ...
  // Navegar al panel de agencia (primero al login)
  irAPanelAgencia(): void {
    // Verificar si ya hay sesión de agencia
    if (this.isBrowser) {
  ...
  ngOnInit(): void {
    ...
    if (this.isBrowser) {
      ...
      // Verificar si viene de unirse a un viaje (query param tab=viajes)
      this.route.queryParams.subscribe(params => {
        if (params['tab'] === 'viajes') {
          this.volverAViajes();
        }
      });
  ...
  private verificarInvitacionPendiente(): void {
    if (!this.isBrowser) return;
    
    const pendingInvite = localStorage.getItem('travelplus_pending_invite');
    if (pendingInvite && this.authService.isLogged()) {
      // Hay una invitación pendiente y el usuario está logueado
      console.log('📨 Invitación pendiente detectada:', pendingInvite);
      
      // Marcar como confirmada y redirigir
      localStorage.setItem('travelplus_invite_confirmed_' + pendingInvite, 'true');
      
      // Redirigir al componente de unirse al viaje
      this.router.navigate(['/unirse-viaje', pendingInvite]);
    }
  }
