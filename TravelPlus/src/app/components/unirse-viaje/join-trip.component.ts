...
export class JoinTripComponent implements OnInit {
  ...
  viajeData: any = null; // Datos completos del viaje
  ...
  private obtenerInfoViaje(): void {
    // No requiere autenticación para obtener info de la invitación
    this.http.get<any>(`${this.apiUrl}/invitaciones/${this.codigoInvitacion}`).subscribe({
      next: (response) => {
  ...
  validarInvitacion(): void {
    ...
    console.log('🔍 Validando invitación:', this.codigoInvitacion);

    // Primero intentar validar desde el backend (sin autenticación requerida)
    this.http.get<any>(`${this.apiUrl}/invitaciones/${this.codigoInvitacion}`).subscribe({
      next: (response) => {
        ...
        if (response && response.invitacion && response.invitacion.viaje_id) {
          ...
          console.log('⚠️ Respuesta sin datos de invitación, intentando localStorage');
          this.validarDesdeLocalStorage();
        }
      },
      error: (err) => {
        console.error('❌ Error al validar invitación:', err);
        // Si el error es 404, la invitación no existe
        if (err.status === 404) {
  ...
  unirseAlViaje(): void {
    ...
    // Usar el nuevo endpoint que vincula al usuario con el viaje
    this.http.post<any>(`${this.apiUrl}/viajes/unirse`, { codigo: this.codigoInvitacion }, this.getAuthHeaders()).subscribe({
      next: (res) => {
        this.cargando = false;
        this.yaUnido = true;
        this.mensaje = res.message || '¡Te has unido exitosamente!';
        this.viajeId = res.viajeId;
        
        if (res.viaje) {
          this.nombreViaje = res.viaje.nombre;
          this.viajeData = res.viaje; // Guardar datos completos del viaje
        }
      ...
      error: (err) => {
        ...
        if (err.error?.error === 'Ya eres el dueño de este viaje' || err.error?.message?.includes('Ya estás unido')) {
          this.yaUnido = true;
          this.mensaje = err.error?.message || 'Ya estás en este viaje';
          ...
          this.mensaje = 'La invitación no es válida o ha expirado';
          this.errorMensaje = true;
        } else {
          // Intentar con el fallback al endpoint antiguo
          this.unirseConFallback();
        }
      }
    });
  }

  private unirseConFallback(): void {
    if (!this.viajeId) {
    ...
    this.http.post<any>(`${this.apiUrl}/viajes/${this.viajeId}/participantes`, participanteData, this.getAuthHeaders()).subscribe({
      next: () => {
        this.cargando = false;
        this.yaUnido = true;
      ...
      error: () => {
        // Agregar localmente aunque falle el backend
        if (this.isBrowser) {
          localStorage.removeItem('travelplus_pending_invite');
          localStorage.removeItem('travelplus_invite_confirmed_' + this.codigoInvitacion);
        }
  ...
  private agregarViajeAColecciones(): void {
    ...
    try {
      ...
      if (!colecciones.find((c: any) => c.id === this.viajeId)) {
        // Agregar viaje con datos completos para que se muestre correctamente
        colecciones.push({
          id: this.viajeId,
          nombre: this.nombreViaje || 'Viaje compartido',
          icono: this.viajeData?.icono || '✈️',
          destinos: this.viajeData?.destinos || [],
          fechaCreacion: Date.now(),
          fechaInicio: this.viajeData?.fecha_inicio,
          fechaFin: this.viajeData?.fecha_fin,
          esCompartido: true
        });
        localStorage.setItem(storageKey, JSON.stringify(colecciones));
        console.log('✅ Viaje agregado a colecciones locales:', this.viajeId);
      }
    } catch (e) {
  ...
  irAlViaje(): void {
    // Navegar al home con la sección de viajes activa
    this.router.navigate(['/home'], { queryParams: { tab: 'viajes' } });
  }
  ...
  private cargarDatosUsuarioActual(): void {
    ...
    if (user) {
      ...
      if (user.uid && this.isBrowser) {
        this.http.get<any>(`${this.apiUrl}/usuarios/${user.uid}`, this.getAuthHeaders()).subscribe({
          next: (usuario) => {
